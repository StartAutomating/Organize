<#
.Synopsis
    GitHub Action for Organize
.Description
    GitHub Action for Organize.  This will:

    * Import Organize
    * If `-Run` is provided, run that script
    * Otherwise, unless `-SkipScriptFile` is passed, run all *.Organize.ps1 files beneath the workflow directory
...
param(
# A PowerShell Script that uses Organize.  
# Any files outputted from the script will be added to the repository.
# If those files have a .Message attached to them, they will be committed with that message.
[string]
$Run,

# If set, will not process any files named *.Organize.ps1
[switch]
$SkipScriptFile,
